...
def MachOPointer(mem_off, subcon, macho=None):
    ...
    def _obtain_physical_offset(ctx):
        ...
        if _macho is None:
            raise malstruct.ConstructError('Input file is not Mach-O')
        ...
        for idx in range(_macho.size):
            ...
            if mbin.is_valid_addr(_mem_off):
                ...
                if offset != 0xffffffffffffffff:
                    return offset
        raise malstruct.ConstructError('Unable to decode virtual address')

    return malstruct.Pointer(_obtain_physical_offset, subcon)
...
class MachOMemoryAddress(malstruct.Adapter):
    r"""
    ...
    def _decode(self, obj, context, path):
        ...
        if address is None:
            raise malstruct.ConstructError('Unable to encode physical address.')
        return address


class MachOFatMemoryAddress(malstruct.Adapter):
    r"""
