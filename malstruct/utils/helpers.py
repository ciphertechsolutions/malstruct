...
from malstruct.lib.py3compat import unicodestringtype
from malstruct.debug import Probe
...
class Base64(Adapter):
    ...
    def _encode(self, obj, context, path):
        from malstruct.utils import custombase64
...
def ELFPointer(mem_off, subcon, elf=None):
    ...
    def _obtain_physical_offset(ctx):
        from malstruct.utils import elffileutils
...
class PEPhysicalAddress(Adapter):
    ...
    def _encode(self, obj, context, path):
        from malstruct.utils import pefileutils
...
def find_constructs(struct, data):
    ...
    while True:
        ...
        except (ConstructError, OverflowError) as e:
            data.seek(offset + 1)
        else:
