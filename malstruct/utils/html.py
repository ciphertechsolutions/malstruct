...
class MemberMap(Adapter):
    ...
    def _wrap_subcon(self, subcon):
        ...
        if isinstance(subcon, malstruct) and not isinstance(subcon, Adapter):
...
if __name__ == '__main__':
    ...
    from utils.network import IP4Address
    from utils.helpers import HexString
    ...
    address_struct = Struct(
        'first' / Struct('a' / Byte, 'b' / Byte),
        'second' / Struct('inner2' / Bytes(2))
        # 'internal' / IP4Address
    )

    PACKET = Struct(
        Padding(0x9),
        'Hardcoded Value 1' / HexString(Int32ul),
        'Hardcoded Value 2' / HexString(Int32ul),
        'Hardcoded Value 3' / HexString(Int32ul),
        Padding(0x17),
        'Compromised Host IP' / IP4Address,  # Use IP adapter
        # 'Unknown IP Addresses' / Switch(
        ...
        Padding(8),
        'Unknown Indicator' / String(0xF),
        Padding(2),
        'Number of CPUs' / Int32ul,
        'CPU Mhz' / Int32ul,
        'Total Memory (MB)' / Int32ul,
        'Compromised System Kernel' / CString(),
        'Possible Trojan Version' / CString()
    )
