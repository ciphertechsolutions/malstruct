...
def test_pe32():
    if (data := read_file_data(SAMPLES_PATH / "test32.exe")) and (
        ...
        assert PEImport(Int32ul, pe=pe).parse(b"\x04\x20\x40\x00") == "WriteConsoleA"
        assert PEImport(Int32ul, pe=pe).build("WriteConsoleA") == b"\x04\x20\x40\x00"
        assert PEImportSymbol.parse(b"\x04\x20\x40\x00", pe=pe) == "WriteConsoleA"
    ...
    if (data := read_file_data(SAMPLES_PATH / "test64.exe")) and (
        ...
        with io.BytesIO(data) as stream:
            ...
            spec = Struct(
                "relative_offset" / Int32ul,
                "instruction_end" / PEMemoryAddress(Tell),
                "name" / PEImportPointer(this.relative_offset + this.instruction_end),
            )
            assert spec.parse_stream(stream, pe=pe).name == "WriteConsoleA"
