...
def test_bytewise(common):
    common(Bitwise(Bytewise(Bytes(1))), b"\xff", b"\xff", 1)
    common(
        BitStruct("p1" / Nibble, "num" / Bytewise(Int24ub), "p2" / Nibble),
        b"\xf0\x10\x20\x3f",
        Container(p1=15, num=0x010203, p2=15),
        4,
    )
    common(
        Bitwise(Sequence(Nibble, Bytewise(Int24ub), Nibble)),
        b"\xf0\x10\x20\x3f",
        [0x0F, 0x010203, 0x0F],
        4,
    )
...
def test_floats(common):
    common(Half, b"\x00\x00", 0.0, 2)
    common(Half, b"\x35\x55", 0.333251953125, 2)
    common(Single, b"\x00\x00\x00\x00", 0.0, 4)
    common(Single, b"?\x99\x99\x9a", 1.2000000476837158, 4)
    common(Double, b"\x00\x00\x00\x00\x00\x00\x00\x00", 0.0, 8)
    common(Double, b"?\xf3333333", 1.2, 8)


def test_formatfield(common, raises):
    d = FormatField("<", "L")
...
def test_bytesinteger(common, raises):
    d = BytesInteger(0)
...
def test_bitsinteger(common, raises):
    d = BitsInteger(0)
    assert raises(d.parse, b"") == IntegerError
    assert raises(d.build, 0) == IntegerError
    d = BitsInteger(8)
    ...
    common(
        d,
        b"\x00\x00\x00\x00\x00\x00\x00\x00\x01\x01\x01\x01\x01\x01\x01\x01",
        0xFF00,
        16,
    )
...
def test_varint(common, raises):
    d = VarInt
    common(
        d,
        b"\x80\x80\x80\x80\x80\x80\x80\x80\x80\x80\x80\x80\x80\x80\x80\x80\x80\x10",
        2**123,
        SizeofError,
    )
    ...
    for n in range(0, 127):
        common(d, bytes([n]), n, SizeofError)
    assert raises(d.parse, b"") == StreamError
...
def test_zigzag(common, raises):
    ...
    for n in [0, 1, 5, 100, 255, 256, 65535, 65536, 2**32, 2**100]:
        assert d.parse(d.build(n)) == n
    for n in range(0, 63):
        common(d, bytes([n * 2]), n, SizeofError)
    assert raises(d.parse, b"") == StreamError
    assert raises(d.build, None) == IntegerError
    assert raises(d.sizeof) == SizeofError
...
def test_paddedstring(common, raises):
    common(PaddedString(10, "utf8"), b"hello\x00\x00\x00\x00\x00", "hello", 10)

    d = PaddedString(100, "ascii")
    assert d.parse(b"X" * 100) == "X" * 100
    assert d.build("X" * 100) == b"X" * 100
    assert raises(d.build, "X" * 200) == PaddingError
...
def test_pascalstring(common, raises):
    for e, us in [
        ("utf8", 1),
        ("utf16", 2),
        ("utf_16_le", 2),
        ...
        for sc in [Byte, Int16ub, Int16ul, VarInt]:
            ...
            common(PascalString(sc, e), sc.build(0), "")
...
def test_pascalstring_issue_960(raises):
    d = Select(PascalString(Byte, "ascii"))
    assert raises(d.parse, b"\x01\xff") == SelectError
    assert raises(d.build, "Афон") == SelectError


def test_cstring(common, raises):
    for e, us in [
        ("utf8", 1),
        ("utf16", 2),
        ("utf_16_le", 2),
        ...
        common(CString(e), bytes(us), "")

        assert CString("utf8").build(s) == b"\xd0\x90\xd1\x84\xd0\xbe\xd0\xbd" + b"\x00"
        assert (
            CString("utf16").build(s)
            == b"\xff\xfe\x10\x04D\x04>\x04=\x04" + b"\x00\x00"
        )
        assert (
            CString("utf32").build(s)
            == b"\xff\xfe\x00\x00\x10\x04\x00\x00D\x04\x00\x00>\x04\x00\x00=\x04\x00\x00"
            + b"\x00\x00\x00\x00"
        )
...
def test_greedystring(common, raises):
    for e, us in [
        ("utf8", 1),
        ("utf16", 2),
        ("utf_16_le", 2),
        ...
        common(GreedyString(e), b"", "")

    for e in ["utf8", "utf16", "utf-16-le", "utf32", "utf-32-le", "ascii"]:
        assert raises(GreedyString(e).sizeof) == SizeofError


def test_string_encodings(common):
    # checks that "-" is replaced with "_"
    common(GreedyString("utf-8"), b"", "")
    common(GreedyString("utf-8"), b"\xd0\x90\xd1\x84\xd0\xbe\xd0\xbd", "Афон")


def test_flag(common):
    d = Flag
...
def test_enum_issue_298(common):
    d = Struct(
        "ctrl"
        / Enum(
            Byte,
...
def test_enum_issue_677(common):
    d = Enum(Byte, one=1)
    common(d, b"\xff", 255, 1)
...
@pytest.mark.xfail(reason="Cannot implement this in EnumIntegerString.")
...
def test_flagsenum(common, raises):
    d = FlagsEnum(Byte, one=1, two=2, four=4, eight=8)
    common(
        d,
        b"\x03",
        Container(_flagsenum=True, one=True, two=True, four=False, eight=False),
        1,
    )
...
def test_flagsenum_enum34(common):
    import enum
...
def test_flagsenum_enum36(common):
    import enum
...
def test_mapping(common):
    x = object
    d = Mapping(Byte, {0: x})
    common(d, b"\x00", x, 1)


def test_struct(common, raises):
    ...
    common(
        Struct(Const(b"\x00"), Padding(1), Pass, Terminated), bytes(2), {}, SizeofError
    )
...
def test_struct_nested(common):
    d = Struct("a" / Byte, "b" / Int16ub, "inner" / Struct("c" / Byte, "d" / Byte))
    common(
        d, b"\x01\x00\x02\x03\x04", Container(a=1, b=2, inner=Container(c=3, d=4)), 5
    )


def test_struct_kwctor(common):
...
def test_struct_proper_context():
    ...
    assert d.parse(b"\x01\x0f") == Container(
        x=1, inner=Container(y=15, a=2, b=17), c=4, d=19
    )
...
@pytest.mark.xfail(ONWINDOWS, reason="/dev/zero not available on Windows")
def test_array_nontellable():
    assert Array(5, Byte).parse_stream(devzero) == [0, 0, 0, 0, 0]


def test_greedyrange(common):
    d = GreedyRange(Byte)
...
def test_repeatuntil(common, raises):
...
def test_computed(common, raises):
...
@pytest.mark.xfail(
    reason="_index fails during parsing or building, not during compilation"
)
def test_index(common):
    ...
    common(
        d,
        b"abbccc",
        [Container(i=0, d=b"a"), Container(i=1, d=b"bb"), Container(i=2, d=b"ccc")],
    )
...
def test_rebuild_custom_function():
    ...
    assert template.parse(b"\x02ab") == Container(count=2, my_items=[97, 98])
    assert template.build(dict(count=None, my_items=[255, 255])) == b"\x02\xff\xff"
    assert template.build(dict(count=2, my_items=[255, 255])) == b"\x02\xff\xff"
    assert template.build(dict(my_items=[255, 255])) == b"\x02\xff\xff"


def test_default(common):
    d = Default(Byte, 0)
    common(d, b"\xff", 255, 1)
    assert d.build(None) == b"\x00"


def test_check(common, raises):
...
def test_error(common, raises):
    assert raises(Error.parse, b"") == ExplicitError
    assert raises(Error.build, None) == ExplicitError
    assert ("x" / Int8sb >> IfThenElse(this.x > 0, Int8sb, Error)).parse(
        b"\x01\x05"
    ) == [1, 5]
    assert (
        raises(
            ("x" / Int8sb >> IfThenElse(this.x > 0, Int8sb, Error)).parse, b"\xff\x05"
        )
        == ExplicitError
    )


def test_focusedseq(common, raises):
    common(
        FocusedSeq("num", Const(b"MZ"), "num" / Byte, Terminated),
        b"MZ\xff",
        255,
        SizeofError,
    )
    common(
        FocusedSeq(this._.s, Const(b"MZ"), "num" / Byte, Terminated),
        b"MZ\xff",
        255,
        SizeofError,
        s="num",
...
def test_namedtuple(common, raises):
    ...
    assert (
        raises(
            lambda: NamedTuple(
                "coord", "x y z", BitStruct("x" / Byte, "y" / Byte, "z" / Byte)
            )
        )
        == NamedTupleError
    )
...
def test_timestamp(common, raises):
    import arrow
...
def test_hex(common):
    ...
    common(
        d,
        b"\x00\x00\x01\x02",
        dict(data=b"\x00\x00\x01\x02", value=0x0102, offset1=0, offset2=4, length=4),
        4,
    )
    obj = d.parse(b"\x00\x00\x01\x02")
    assert str(obj) == "unhexlify('00000102')"
...
def test_union_issue_348():
    ...
    assert d.parse(b"\x00\x04\x11\x22\x33\x44") == {
        "Int16": [4386, 13124],
        "Int32": [287454020],
        "Int8": [17, 34, 51, 68],
    }
...
def test_optional_in_struct_issue_747(raises):
...
def test_optional_in_bit_struct_issue_747(raises):
...
def test_alignedstruct(common):
    d = AlignedStruct(4, "a" / Int8ub, "b" / Int16ub)
    common(d, b"\x01\x00\x00\x00\x00\x05\x00\x00", Container(a=1, b=5), 8)


def test_bitstruct(common):
    d = BitStruct(
        "a" / BitsInteger(3), "b" / Flag, Padding(3), "c" / Nibble, "d" / BitsInteger(5)
    )
...
def test_offsettedend(common, raises):
    d = Struct(
        "header" / Bytes(2),
...
def test_seek(raises):
    ...
    assert (Bytes(10) >> d >> Byte).build(
        [b"0123456789", None, 255]
    ) == b"01234\xff6789"
    assert Struct("data" / Bytes(10), d, "addin" / Byte).parse(
        b"0123456789"
    ) == Container(data=b"0123456789", addin=53)
    assert (
        Struct("data" / Bytes(10), d, "addin" / Byte).build(
            dict(data=b"0123456789", addin=53)
        )
        == b"01234\x356789"
    )
    assert (Seek(10, 1) >> Seek(-5, 1) >> Bytes(1)).parse(b"0123456789") == [
        10,
        5,
        b"5",
    ]
    assert (Seek(10, 1) >> Seek(-5, 1) >> Bytes(1)).build(
        [None, None, 255]
...
def test_rawcopy():
    d = RawCopy(Byte)
    assert d.parse(b"\xff") == dict(
        data=b"\xff", value=255, offset1=0, offset2=1, length=1
    )
...
def test_prefixed(common, raises):
    ...
    common(d, b"\x0a" + bytes(10), "\x00" * 10, SizeofError)


def test_prefixedarray(common, raises):
...
def test_fixedsized(common, raises):
    ...
    common(d, bytes(10), "\x00" * 10, 10)


def test_nullterminated(common, raises):
...
def test_restreamdata(common, raises):
...
@pytest.mark.xfail(
    reason="unknown, either StreamError or KeyError due to this.entire or this._.entire"
)
def test_restreamdata_issue_701():
    ...
    assert d.parse(b"\x01GGGGGGGGGG\x02") == Container(
        entire=b"\x01GGGGGGGGGG\x02", ac=Container(a=1, b=2)
    )

    d = FocusedSeq(
        "ac" "entire" / GreedyBytes,
        "ac"
        / RestreamData(
            this.entire,
            Struct(
...
def test_transformed(common, raises):
    ...
    common(d, bytes(2), "\x00" * 16, SizeofError)
...
def test_restreamed(common, raises):
...
def test_restreamed_partial_read(common, raises):
    d = Restreamed(Bytes(255), ident, 1, ident, 1, ident)
    assert raises(d.parse, b"") == StreamError


def test_processxor(common, raises):
    ...
    common(d, b"\x00", "X", SizeofError)
    d = ProcessXor(b"XXXXX", GreedyString("utf-8"))
    common(d, b"\x00", "X", SizeofError)


def test_processrotateleft(common, raises):
...
def test_checksum():
    ...
    assert d.parse(b"\x01\x02" + c) == Container(
        fields=dict(
            data=b"\x01\x02", value=Container(a=1, b=2), offset1=0, offset2=2, length=2
        ),
        checksum=c,
    )
...
def test_checksum_nonbytes_issue_323(common, raises):
    d = Struct(
        "vals" / Byte[2],
...
def test_compressed_zlib(common, raises):
    zeros = bytes(10000)
...
def test_compressed_gzip(common, raises):
    zeros = bytes(10000)
...
def test_compressed_bzip2(common, raises):
    zeros = bytes(10000)
...
def test_compressed_lzma(common, raises):
    zeros = bytes(10000)
...
def test_compressed_prefixed(common, raises):
    zeros = bytes(10000)
    d = Prefixed(VarInt, Compressed(GreedyBytes, "zlib"))
    st = Struct("one" / d, "two" / d)
    assert st.parse(st.build(Container(one=zeros, two=zeros))) == Container(
        one=zeros, two=zeros
    )
    assert raises(d.sizeof) == SizeofError


def test_compressedlz4(common, raises):
    zeros = bytes(10000)
...
def test_encryptedsym(common, raises):
    ...
    # AES 128/256 bit - ECB
    d = EncryptedSym(
        GreedyBytes, lambda ctx: Cipher(algorithms.AES(ctx.key), modes.ECB())
    )
    common(
        d,
        b"\xf4\x0f\x54\xb7\x6a\x7a\xf1\xdb\x92\x73\x14\xde\x2f\xa0\x3e\x2d",
        b"Secret Message..",
        key=key128,
        iv=iv,
    )
    common(
        d,
        b"\x82\x6b\x01\x82\x90\x02\xa1\x9e\x35\x0a\xe2\xc3\xee\x1a\x42\xf5",
        b"Secret Message..",
        key=key256,
        iv=iv,
    )
    ...
    # AES 128/256 bit - CBC
    d = EncryptedSym(
        GreedyBytes, lambda ctx: Cipher(algorithms.AES(ctx.key), modes.CBC(ctx.iv))
    )
    common(
        d,
        b"\xba\x79\xc2\x62\x22\x08\x29\xb9\xfb\xd3\x90\xc4\x04\xb7\x55\x87",
        b"Secret Message..",
        key=key128,
        iv=iv,
    )
    common(
        d,
        b"\x60\xc2\x45\x0d\x7e\x41\xd4\xf8\x85\xd4\x8a\x64\xd1\x45\x49\xe3",
        b"Secret Message..",
        key=key256,
        iv=iv,
    )
    ...
    # AES 128/256 bit - CTR
    d = EncryptedSym(
        GreedyBytes, lambda ctx: Cipher(algorithms.AES(ctx.key), modes.CTR(ctx.nonce))
    )
    common(
        d,
        b"\x80\x78\xb6\x0c\x07\xf5\x0c\x90\xce\xa2\xbf\xcb\x5b\x22\xb9\xb5",
        b"Secret Message..",
        key=key128,
        nonce=nonce,
    )
    common(
        d,
        b"\x6a\xae\x7b\x86\x1a\xa6\xe0\x6a\x49\x02\x02\x1b\xf2\x3c\xd8\x0d",
        b"Secret Message..",
        key=key256,
        nonce=nonce,
    )
...
def test_encryptedsym_cbc_example():
    ...
    d = Struct(
        ...
        "enc_data"
        / EncryptedSym(
            Aligned(16, Struct("width" / Int16ul, "height" / Int16ul)),
...
def test_encryptedsymaead(common, raises):
    ...
    assert (
        raises(EncryptedSymAead(GreedyBytes, "AESGCM", bytes(16)).build, b"")
        == CipherError
    )
    assert (
        raises(EncryptedSymAead(GreedyBytes, "AESGCM", bytes(16)).parse, b"")
        == CipherError
    )
...
def test_encryptedsymaead_gcm_example():
    ...
    byts = d.build(
        {
            "associated_data": b"This is authenticated",
            "enc_data": b"The secret message",
        },
        key=key128,
    )
    obj = d.parse(byts, key=key128)
...
def test_rebuffered(common, raises):
    data = b"0" * 1000
    assert Rebuffered(Array(1000, Byte)).parse_stream(io.BytesIO(data)) == [48] * 1000
    assert (
        Rebuffered(Array(1000, Byte), tailcutoff=50).parse_stream(io.BytesIO(data))
        == [48] * 1000
    )
...
def test_lazy_seek():
    d = Struct("a" / Int8ub, "b" / Lazy(Bytes(2)), "c" / Int16ub, "d" / Lazy(Bytes(4)))
...
def test_lazystruct(common, raises):
    d = LazyStruct(
        "num1" / Int8ub,
        "num2" / BytesInteger(1),
...
def test_lazyarray(common, raises):
...
def test_lazybound(common, raises):
...
def test_exprvalidator(common, raises):
...
def test_operators(common, raises):
    common(Struct("new" / ("old" / Byte)), b"\x01", Container(new=1), 1)
    common(
        Struct(Renamed(Renamed(Byte, newname="old"), newname="new")),
        b"\x01",
        Container(new=1),
        1,
    )
    ...
    common(
        Struct("nums" / Byte[4]), b"\x01\x02\x03\x04", Container(nums=[1, 2, 3, 4]), 4
    )
    ...
    common(
        Struct("count" / Byte, "items" / Byte[this.count], Pass, Terminated),
        b"\x03\x01\x02\x03",
        Container(count=3, items=[1, 2, 3]),
        SizeofError,
    )
    common(
        "count" / Byte + "items" / Byte[this.count] + Pass + Terminated,
        b"\x03\x01\x02\x03",
        Container(count=3, items=[1, 2, 3]),
        SizeofError,
    )
...
def test_operators_issue_87():
    ...
    assert ("unicode_name" / Byte).parse(b"\x01") == 1
    assert (b"bytes_name" / Byte).parse(b"\x01") == 1
    assert (None / Byte).parse(b"\x01") == 1


def test_from_issue_76(common, raises):
...
def test_from_issue_171():
    ...
    assert attributes.parse(blob) == Container(
        attr=[
...
def test_from_issue_71():
    ...
    Outer = Struct(
        ...
        "checksum"
        / Checksum(
            Bytes(64), lambda data: hashlib.sha512(data).digest(), this.payload.data
        ),
...
def test_from_issue_246(common, raises):
    NumVertices = Bitwise(
        Aligned(
            8,
            Struct(
    ...
    testBit = BitStruct("a" / BitsInteger(8), "b" / If(this.a == 97, BitsInteger(8)))
    testByte = Struct("a" / Byte, "b" / If(this.a == 97, Byte))
    ...
    assert NumVertices.parse(b"\x01\x34\x56\x70") == Container(
        numVx4=Container(num=0),
        numVx8=Container(num=19),
        numVx16=Container(num=1262951),
    )
...
def test_from_issue_244():
    ...
    assert d.parse(b"abcd") == [
        Container(num=97, index=0),
        Container(num=98, index=1),
        Container(num=99, index=2),
        Container(num=100, index=3),
    ]
...
def test_hanging_issue_280(common, raises):
...
def test_from_issue_324():
    d = Struct(
        ...
        "checksum" / Checksum(Byte, lambda data: sum(data) & 0xFF, this.vals.data),
    )
...
def test_from_issue_781():
    ...
    assert x.animal == "giraffe"  # works
    assert (
        x.animal == d.animal.giraffe
    )  # AttributeError: 'Transformed' object has no attribute 'animal'


def test_this_expresion_compare_container(common, raises):
    st = Struct(
        "flags" / FlagsEnum(Byte, a=1),
...
def test_exposing_members_context(common, raises):
    d = Struct(
        "count" / Byte,
    ...
    assert d.parse(b"\x01\x02\x03\x04") == dict(
        chars=[1, 2, 3, 4], data=b"\x01\x02\x03\x04"
    )


def test_isparsingbuilding(raises):
    d = Struct(
...
def test_struct_root_topmost():
    ...
    # d.parse(b'', z=2)
    assert d.parse(b"", z=2) == Container(
        x=1, inner=Container(inner2=Container(x=1, z=2, zz=2))
    )
...
def test_parsedhook_repeatersdiscard():
    ...
    d = RepeatUntil(
        lambda obj, lst, ctx: ctx._index == 2, Byte * printobj, discard=True
    )
    assert d.parse(b"\x01\x02\x03") == []
    assert outputs == [1, 2, 3]
...
@pytest.mark.xfail(
    reason="both sizeof fail because length is 1 level up than when parsing"
)
def test_from_issue_692():
    ...
    assert AttReadByTypeResponse.parse(
        b"\x04\x01\x02\x03\x04\x01\x02\x03\x04"
    ) == Container(
        length=4,
        datalist=[
            dict(handle=0x0201, value=b"\x03\x04"),
            dict(handle=0x0201, value=b"\x03\x04"),
        ],
    )
...
def test_showpath2():
    x = Struct(
        ...
        / Struct(
            ...
            "b"
            / Struct(
                "foo" / Bytes(1), "c" / Struct("foo" / Bytes(1), "bar" / Bytes(1))
            ),
        ),
    )
    x.parse(b"\xff" * 5)
...
def test_struct_issue_771():
    spec = Struct(
        "a" / Int32ul, "b" / Struct("count" / Int32ul, "entries" / Byte[this.count])
    )
...
def test_struct_copy(common, raises):
    import copy
...
def test_switch_issue_913_using_enum(common, raises):
...
def test_switch_issue_913_using_strings(common, raises):
    mapping = {
        "Zero": Pass,
        "One": Int8ul,
        "Two": Int16ul,
...
def test_switch_issue_913_using_integers(common, raises):
    mapping = {
        0: Pass,
        1: Int8ul,
        2: Int16ul,
...
@pytest.mark.xfail(reason="unfixable defect in the design")
...
def test_nullterminated_longterm_issue_1046(raises):
    d = NullTerminated(GreedyBytes, term=b"END")
    assert d.parse(b"xxxEND") == b"xxx"
    assert raises(d.parse, b"xENDxx") == StreamError


def test_compile_binexpr_bitwise_and_issue_1039(common, raises):
    d = Struct(
        "a" / Int8ub,
...
@pytest.mark.xfail(reason="unknown problem with Select")
...
def test_issue_1014():
    ...
    assert d.parse(bytes([1, 2, 3, 4, 5, 6])) == Container(
        version=0x0102, box=Container(position=3, payload=b"\x04\x05\x06")
    )
...
def test_issue_1082():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        outPuter = {"Linux": "cat", "Darwin": "cat", "Windows": "type"}[
            platform.system()
        ]
        ...
        for d in [Prefixed(Int8ub, Byte), FixedSized(2, Byte)]:
            pid = subprocess.Popen(
                f"{outPuter} {oFileName}", stdout=subprocess.PIPE, shell=True
            )
