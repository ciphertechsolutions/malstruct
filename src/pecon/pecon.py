...
class Container(malstruct.Container):
...
class DataDirectories(malstruct.ListContainer):
    """
    ...
    def sizeof(self):
        return malstruct.IMAGE_DATA_DIRECTORY.sizeof() * len(self)

    # Provide convenience properties for accessing standard data directories.

    @property
    def exports(self):
        return self[malstruct.DATA_DIR_INDEX_EXPORTS]

    @exports.setter
    def exports(self, value):
        self[malstruct.DATA_DIR_INDEX_EXPORTS] = value

    @property
    def imports(self):
        return self[malstruct.DATA_DIR_INDEX_IMPORTS]

    @imports.setter
    def imports(self, value):
        self[malstruct.DATA_DIR_INDEX_IMPORTS] = value

    @property
    def resource(self):
        return self[malstruct.DATA_DIR_INDEX_RESOURCE]

    @resource.setter
    def resource(self, value):
        self[malstruct.DATA_DIR_INDEX_RESOURCE] = value

    @property
    def exception(self):
        return self[malstruct.DATA_DIR_INDEX_EXCEPTION]

    @exception.setter
    def exception(self, value):
        self[malstruct.DATA_DIR_INDEX_EXCEPTION] = value

    @property
    def certificate(self):
        return self[malstruct.DATA_DIR_INDEX_CERTIFICATE]

    @certificate.setter
    def certificate(self, value):
        self[malstruct.DATA_DIR_INDEX_CERTIFICATE] = value

    @property
    def base_reloc(self):
        return self[malstruct.DATA_DIR_INDEX_BASE_RELOC]

    @base_reloc.setter
    def base_reloc(self, value):
        self[malstruct.DATA_DIR_INDEX_BASE_RELOC] = value

    @property
    def debug(self):
        return self[malstruct.DATA_DIR_INDEX_DEBUG]

    @debug.setter
    def debug(self, value):
        self[malstruct.DATA_DIR_INDEX_DEBUG] = value

    @property
    def architecture(self):
        return self[malstruct.DATA_DIR_INDEX_ARCHITECTURE]

    @architecture.setter
    def architecture(self, value):
        self[malstruct.DATA_DIR_INDEX_ARCHITECTURE] = value

    @property
    def global_ptr(self):
        return self[malstruct.DATA_DIR_INDEX_GLOBAL_PTR]

    @global_ptr.setter
    def global_ptr(self, value):
        self[malstruct.DATA_DIR_INDEX_GLOBAL_PTR] = value

    @property
    def tls(self):
        return self[malstruct.DATA_DIR_INDEX_TLS]

    @tls.setter
    def tls(self, value):
        self[malstruct.DATA_DIR_INDEX_TLS] = value

    @property
    def load_config(self):
        return self[malstruct.DATA_DIR_INDEX_LOAD_CONFIG]

    @load_config.setter
    def load_config(self, value):
        self[malstruct.DATA_DIR_INDEX_LOAD_CONFIG] = value

    @property
    def bound_import(self):
        return self[malstruct.DATA_DIR_INDEX_BOUND_IMPORT]

    @bound_import.setter
    def bound_import(self, value):
        self[malstruct.DATA_DIR_INDEX_BOUND_IMPORT] = value

    @property
    def import_address(self):
        return self[malstruct.DATA_DIR_INDEX_IMPORT_ADDRESS]

    @import_address.setter
    def import_address(self, value):
        self[malstruct.DATA_DIR_INDEX_IMPORT_ADDRESS] = value

    @property
    def dely_import_descriptor(self):
        return self[malstruct.DATA_DIR_INDEX_DELAY_IMPORT_DESCRIPTOR]

    @dely_import_descriptor.setter
    def dely_import_descriptor(self, value):
        self[malstruct.DATA_DIR_INDEX_DELAY_IMPORT_DESCRIPTOR] = value

    @property
    def clr_header(self):
        return self[malstruct.DATA_DIR_INDEX_CLR_HEADER]

    @clr_header.setter
    def clr_header(self, value):
        self[malstruct.DATA_DIR_INDEX_CLR_HEADER] = value
...
class Section(Container):
    ...
    def __init__(self, *args, **kw):
        ...
        super().__init__(_section_header)
        for arg in args:
            self.update(arg)
        self.update(kw)
...
class PE(Container):
    ...
    def __init__(self, data=None, is_64bit=False):
        ...
        _characteristics = [
            malstruct.IMAGE_FILE_RELOCS_STRIPPED,
            malstruct.IMAGE_FILE_EXECUTABLE_IMAGE,
            malstruct.IMAGE_FILE_LINE_NUMS_STRIPPED,
            malstruct.IMAGE_FILE_LOCAL_SYMS_STRIPPED,
        ]

        if is_64bit:
            _magic = malstruct.IMAGE_NT_OPTIONAL_HDR64_MAGIC
            _machine = malstruct.IMAGE_FILE_MACHINE_AMD64
        else:
            _magic = malstruct.IMAGE_NT_OPTIONAL_HDR32_MAGIC
            _machine = malstruct.IMAGE_FILE_MACHINE_I386
            _characteristics.append(malstruct.IMAGE_FILE_32BIT_MACHINE)
        ...
        _optional_header = Container(
            {
                ...
                "SizeOfHeaders": 0,
                "CheckSum": 0,
                "Subsystem": malstruct.IMAGE_SUBSYSTEM_WINDOWS_CUI,
                "DllCharacteristics": [],
        ...
        _file_header = Container(
            {
                ...
                "NumberOfSymbols": 0,
                "SizeOfOptionalHeader": malstruct.IMAGE_OPTIONAL_HEADER.sizeof(
                    **_optional_header
                ),
        ...
        self.SectionTable = malstruct.ListContainer()

    def _parse(self, data):
        """
        Parses data containing PE file and updates dictionary to reflect results.

        :raises malstructError: If provided data could not be parsed (ie. not a pe)
        """
        pe = malstruct.PEFILE_HEADER.parse(data)
        ...
        for section in pe.SectionTable:
            if isinstance(section.Characteristics, dict):
                section.Characteristics = [
                    flag for flag, value in section.Characteristics.items() if value
                ]
            section.data = data[
        ...
        if isinstance(optional_header.DllCharacteristics, dict):
            optional_header.DllCharacteristics = [
                flag
                for flag, value in optional_header.DllCharacteristics.items()
                if value
            ]
    ...
    def _fix_section(self, section):
        ...
        if not section_copy.Characteristics:
            _characteristics = [malstruct.IMAGE_SCN_MEM_READ]
            if section_copy.Name == ".text":
                _characteristics += [
                    malstruct.IMAGE_SCN_CNT_CODE,
                    malstruct.IMAGE_SCN_MEM_EXECUTE,
                ]
            else:
                _characteristics += [malstruct.IMAGE_SCN_CNT_INITIALIZED_DATA]
            if section_copy.Name == ".data":
                _characteristics += [malstruct.IMAGE_SCN_MEM_WRITE]
            if section_copy.Name == ".reloc":
                _characteristics += [malstruct.IMAGE_SCN_MEM_DISCARDABLE]
        ...
        if isinstance(section_copy.Characteristics, list):
            section_copy.Characteristics = {
                flag: True for flag in section_copy.Characteristics
            }
    ...
    def build(self, write_section_data=True):
        ...
        dos_stub_size = pe.DosHeader.e_lfanew - malstruct.IMAGE_DOS_HEADER.sizeof()
        if len(pe.DosStub) > dos_stub_size:
            raise ValueError(
                "Provided DOS stub is too large for provided DosHeader.e_lfanew: {}".format(
        ...
        if isinstance(file_header.Characteristics, list):
            file_header.Characteristics = {
                flag: True for flag in file_header.Characteristics
            }
        ...
        file_header.SizeOfOptionalHeader = malstruct.IMAGE_OPTIONAL_HEADER.sizeof(
            **optional_header
        )
        if isinstance(optional_header.DllCharacteristics, list):
            optional_header.DllCharacteristics = {
                flag: True for flag in optional_header.DllCharacteristics
            }

        # SizeOfHeaders is the sum of the headers rounded by FileAlignment.
        headers_size = malstruct.PEFILE_HEADER.sizeof(**pe)
        ...
        pe_data = malstruct.PEFILE_HEADER.build(pe)

        # Add section data.
        if write_section_data:
            stream = io.BytesIO(pe_data)
            spec = malstruct.Pointer(
                this.PointerToRawData, malstruct.Bytes(this.SizeOfRawData)
            )
            for section in pe.SectionTable:
