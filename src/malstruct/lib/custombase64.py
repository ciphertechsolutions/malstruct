...
def _adjust_pad(alphabet, data, decode):
    logger.warning(
        "The padding character has not been specified in the custom alphabet"
    )
    ...
    if decode:
        for char in data:
            if char not in alphabet:
                logger.info(
                    ...
                    "but was found in the encoded data.  it will be used as the padding char".format(
                        char
                    )
                )
                return (
                    alphabet + bytes([char]) if isinstance(char, int) else char
        ...
        if b"=" not in alphabet:
            ...
            raise ValueError(
                "ERROR: please provide a padding character to the custom alphabet"
            )
