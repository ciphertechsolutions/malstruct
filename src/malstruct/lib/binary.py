...
BITS2BYTES_CACHE = {bytes2bits(bytes([i])): i for i in range(256)}
...
def swapbytesinbits(data):
    ...
    if len(data) % 8 != 0:
        raise ValueError(
            f"little-endianness is only defined if data length {len(data)} is multiple of 8"
        )
    return b"".join(data[i : i + 8] for i in reversed(range(0, len(data), 8)))


SWAPBITSINBYTES_CACHE = {
    i: int.from_bytes(bits2bytes(swapbytes(bytes2bits(bytes([i])))), "little")
    for i in range(256)
}
