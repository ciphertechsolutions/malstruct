...
IMAGE_EXPORT_DIRECTORY = malstruct.Struct(
    ...
    "TimeDateStamp" / malstruct.EpochTime,
...
IMAGE_OPTIONAL_HEADER = malstruct.Struct(
    ...
    "BaseOfData"
    / malstruct.If(this.Magic != IMAGE_NT_OPTIONAL_HDR64_MAGIC, malstruct.Int32ul),
    "ImageBase"
    ...
    "NumberOfRvaAndSizes"
    / malstruct.Rebuild(malstruct.Int32ul, malstruct.len_(this.DataDirectory)),
    "DataDirectory"
    / malstruct.Default(
        IMAGE_DATA_DIRECTORY[this.NumberOfRvaAndSizes], DEFAULT_DATA_DIRECTORIES[:]
    ),
)
...
IMAGE_FILE_HEADER = malstruct.Struct(
    ...
    / malstruct.Default(
        malstruct.Int16ul,
        IMAGE_OPTIONAL_HEADER.sizeof(
            Magic=IMAGE_NT_OPTIONAL_HDR32_MAGIC, NumberOfRvaAndSizes=16
        ),
    ),
    "Characteristics"
    / malstruct.FlagsEnum(
...
PUBLICKEYSTRUC = malstruct.Struct(
    ...
    "algid" / AlgorithmID(malstruct.Int32ul),
)
    ...
    def _decode(self, obj, context, path):
        try:
            return datetime.datetime(
                ...
                obj.wMinute,
                obj.wSecond,
                obj.wMilliseconds * 1000,
                tzinfo=self._tzinfo,
            ).isoformat()
        except OSError as e:
            raise malstruct.ConstructError(e)
...
class FileTimeAdapter(malstruct.Adapter):
    ...
    def _decode(self, obj, context, path):
        timestamp = (obj - EPOCH_AS_FILETIME) / HUNDREDS_OF_NANOSECONDS
        if timestamp < 0:
            raise malstruct.ConstructError("Invalid timestamp")
        try:
            return datetime.datetime.fromtimestamp(timestamp, tz=self._tz).isoformat()
        except OSError as e:
            raise malstruct.ConstructError(e)
