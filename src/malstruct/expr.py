...
class UniExpr(ExprMixin):
    ...
    def __repr__(self):
        return "{} {!r}".format(opnames[self.op], self.operand)

    def __str__(self):
        return "{} {}".format(opnames[self.op], self.operand)
...
class BinExpr(ExprMixin):
    ...
    def __repr__(self):
        return "({!r} {} {!r})".format(self.lhs, opnames[self.op], self.rhs)

    def __str__(self):
        return "({} {} {})".format(self.lhs, opnames[self.op], self.rhs)
...
class Path(ExprMixin):
    ...
    def __repr__(self):
        if self.__parent is None:
            ...
            return "{}[{!r}]".format(self.__parent, self.__field)
...
class Path2(ExprMixin):
    ...
    def __repr__(self):
        if self.__parent is None:
            ...
            return "{!r}[{!r}]".format(self.__parent, self.__index)
...
class FuncPath(ExprMixin):
    ...
    def __repr__(self):
        if self.__operand is None:
            ...
            return "{}_({!r})".format(self.__func.__name__, self.__operand)

    def __str__(self):
        if self.__operand is None:
            return "%s_" % (self.__func.__name__)
        else:
            return "{}_({})".format(self.__func.__name__, self.__operand)
    ...
    def __call__(self, operand, *args):
        if self.__operand is None:
            ...
            return self.__func(
                self.__operand(operand) if callable(self.__operand) else self.__operand
            )
