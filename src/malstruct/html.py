...
from .transforms import RawCopy
...
def brightness(hexcode):
    ...
    return (
        int(hexcode[1:3], 16) * 0.299
        + int(hexcode[3:5], 16) * 0.587
        + int(hexcode[5:7], 16) * 0.114
    )


# Calculate brightness for each color and determine if text should be black or white.
FORMAT_COLORS = [
    (bg_color, "#000000" if brightness(bg_color) >= 128 else "#ffffff")
    for bg_color in COLORPALLETTE
]
...
class Member(RawCopy):
    """
    This is a submalstruct that collects offset, data, and size information into the given
...
class MemberMap(Adapter):
    ...
    :param subcon: the malstruct to wrap
    """
    ...
    def _wrap_subcon(self, subcon):
        """Recursively wraps all submalstructs with Member."""
        # Don't wrap the Probes.
        if isinstance(subcon, Probe):
            return subcon

        # Recursively wrap internals as until we hit an adapter or non-malstruct object.
        if isinstance(subcon, Construct) and not isinstance(subcon, Adapter):
...
def _gen_color_map(member_map, depth=1, member_callback=None):
    ...
    for offset, members in sorted(member_map.items()):
        members = list(
            reversed(members)
        )  # Members are generated in reverse with most depth being first.
    ...
    # (This happens when the first entry in a malstruct is empty.)
    visible_members = {
        offset: member
        for offset, member in visible_members.items()
        if id(member) not in blacklist
    }

    # Assign colors.
    color_generator = itertools.cycle(FORMAT_COLORS)
    color_map = {
        offset: (next(color_generator), member)
        for offset, member in sorted(visible_members.items())
    }
...
def html_hex(struct, data, width=16, depth=None, member_callback=None):
    ...
    :param struct: A malstruct object to parse.
    ...
    :param member_callback:
        ...
        member name or value in the variable table. Function must accept two parameters (name, value) and return a tuple of
    ...
    :rtype: str
    :returns: returns unicode string of html data.
    :raises malstructError: If given struct fails to parse given data.
    """
    if width <= 0:
    ...
    for line_number, line in enumerate(
        grouper(width, _iter_colors(data, color_map), fillvalue=(None, None))
    ):
        ...
        for byte, color in line:
            ...
            if color:
                if color != current_color:
                    ...
                    if current_color:
                        prefix += "</span>"
                    prefix += '<span style="background:{};color:{}">'.format(
                        bg_color, text_color
                    )
                    current_color = color
            # Clear highlighting.
